...
import { onCreatePost } from '../CreatePost/actionCreatePostScreen'

export default function ScreenCreateReels() {
  const [selectedImages, setSelectedImages] = useState()
  const [title, setTitle] = useState('')
  const [description, setDescription] = useState('')
  const [postAsTweet, setPostAsTweet] = useState(false)
  const { isLoading, postCreated } = useSelector(state => state.reducerCreatePostScreen)
  ...
  const createPost = async () => {
    ...
    dispatch(onCreatePost({ requestBody, navigation, post: false }))
    setDescription('')
    setSelectedImages('')
  }
